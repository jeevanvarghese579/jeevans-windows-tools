...
Jeevan's Windows Tools does not contain networking, downloading, telemetry, auto-update, driver installation, service installation, or code injection. It normally runs as the current user (`asInvoker`). Momentum Scroll settings offer an explicit **Restart as administrator** button so the user can choose to scroll elevated applications; Windows displays the standard UAC prompt before that restart.
...
Version 1.2 uses one shared low-level mouse hook for all three features. Injected mouse input is ignored. A raw-input listener identifies genuine HID mouse-wheel packets so Momentum Scroll does not add momentum to precision-touchpad gestures; it does not install another low-level hook or suppress input. A separate keyboard hook is active only when the user configures an edge for Window Switching, where Escape must cancel the switcher.

The shared hook replaces the former per-feature mouse hooks and is stopped when the master switch is off. If only Taskbar Switcher is enabled with both edges set to Nothing, no global low-level input hook is installed.
